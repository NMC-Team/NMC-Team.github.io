...
if (data) {
    ...
    const kdMembers = scored.filter(m => m.kd != null);
    const avgKd = kdMembers.length ? mean(kdMembers.map(m => m.kd)) : 0;
    ...
    setMetric('kKd', kdMembers.length ? avgKd.toFixed(2) : '—', kdMembers.length ? `Target: ≥ ${kdTarget.toFixed(2)}` : 'K/D is not supplied by the current CoD API', !kdMembers.length ? '' : avgKd >= kdTarget ? 'text-emerald' : 'text-rose');
    ...
    const render = () => {
        ...
        body.innerHTML = rows.map(m => {
            ...
            return `<tr>
                ...
                <td class="num-cell ${m.kd != null && m.kd >= kdTarget ? '' : 'text-rose'}">${m.kd == null ? '—' : m.kd.toFixed(1)}</td>
