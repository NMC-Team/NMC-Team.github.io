...
import { clearToken, getAuth, isAuthenticated, openCodLogin, getIdentity } from './cod-api.js';
...
export function memberCell(m) {
    const avatar = m.avatar
        ? `<img src="${esc(m.avatar)}" alt="" loading="lazy" referrerpolicy="no-referrer">`
        : esc(initial(m.name));
    return `<div class="player-info">
        <div class="player-avatar${m.avatar ? ' has-avatar' : ''}" aria-hidden="true">${avatar}</div>
...
function mountCodAuth() {
    ...
    const render = () => {
        ...
        if (connected && identity) {
            const saved = getAuth();
            const roleLabel = saved?.roleName ? esc(saved.roleName) : 'Call of Dragons';
            slot.innerHTML = `
                <button class="cod-auth connected" id="codAuthBtn" type="button" title="Call of Dragons live API connected">
                    <span class="cod-auth-dot"></span><span>${roleLabel}</span>
                </button>`;
            $('#codAuthBtn')?.addEventListener('click', async () => {
                const action = window.confirm('Disconnect Call of Dragons from this browser?');
                if (!action) return;
                clearToken();
                window.location.reload();
            });
        } else {
            slot.innerHTML = `<button class="cod-auth" id="codAuthBtn" type="button"><span class="cod-auth-dot"></span><span>Connect CoD</span></button>`;
            $('#codAuthBtn')?.addEventListener('click', async () => {
                const button = $('#codAuthBtn');
                if (button) { button.disabled = true; button.querySelector('span:last-child').textContent = 'Opening…'; }
                try {
                    await openCodLogin();
                    window.location.reload();
                } catch (error) {
                    if (button) { button.disabled = false; button.querySelector('span:last-child').textContent = 'Connect CoD'; }
                    if (error?.message !== 'Login cancelled.') console.error(error);
                }
            });
        }
    };
    ...
    window.addEventListener('storage', event => {
        if (event.key === 'nmc-cod-auth-v1') render();
    });
}
